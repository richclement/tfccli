Read `@specs/PRD.md` and `@specs/review.md`.

## Hard constraints
- **ONLY work on ONE task per iteration.** Do not start a second task, even if you finish early.
- `@specs/review.md` is the **source of truth** for task status and progress notes.
...
Record in `@specs/review.md`: commands run, failures encountered, what changed, final status.

## 1) Pick exactly one task
From `@specs/review.md`, choose the **single highest priority incomplete** task, prioritizing:
- PRD/user impact
- unblockers
...
In `@specs/review.md` under the chosen task, add a brief plan:
- acceptance criteria tied to PRD language
- verification approach (tests/commands)
- 3–7 bullet implementation plan

If the task is too large, **split it in `review.md`** and complete **only the first subtask** this iteration.

If the task is **blocked** (missing requirements, dependency, unclear spec):
- document the blocker in `review.md`
...
## Exit condition
If (and only if) **all tasks in `review.md` are DONE** and the feedback loops are green, output exactly:
